...
public class GsonUtils {
	 ...
	 * @throws GsonSyntaxException 如果JSON字符串格式不正确，将抛出此异常
	 */
	public static JsonElement parse(String json) throws GsonSyntaxException {
	    try {
	        // 使用JsonParser类的静态方法parseString来解析JSON字符串
	        return JsonParser.parseString(json);
	    } catch (Exception ex) {
	        // 捕获解析过程中可能发生的任何异常，并将其包装为GsonSyntaxException重新抛出
	 ...
	 */
	public static JsonObject toJsonObjectWithException(String json) throws GsonSyntaxException {
	 ...
	 * @throws GsonSyntaxException json解析异常
	 */
	public static JsonArray toJsonArrayWithException(String json) throws GsonSyntaxException {
	...
	public <K, V>Map<K, V> toMapWithException(String json, Class<K> classOfK, Class<V> classOfV) throws GsonSyntaxException {
		try {
	...
	public <K, V>Map<K, V> toMapWithException(JsonElement json, Class<K> classOfK, Class<V> classOfV) throws GsonSyntaxException {
		try {
			return fromJsonWithException(json, new TypeFactory(Map.class, classOfK, classOfV));
		} catch(Exception ex) {
			throw new GsonSyntaxException(ex);
	...
	public <T>List<T> toList(String json, Class<T> classOfT) {
		...
		} catch (GsonSyntaxException ignored) {
		}
		return null;
	}
	 ...
	 * @param classOfT classOfT
	 * @return List
	 * @throws GsonSyntaxException GsonSyntaxException
	 */
	public <T>List<T> toListWithException(String json, Class<T> classOfT) throws GsonSyntaxException {
		try {
	...
	public <T>List<T> toListWithException(JsonElement json, Class<T> classOfT) throws GsonSyntaxException {
		try {
			return gson().fromJson(json, new TypeFactory(List.class, classOfT));
		} catch(Exception ex) {
			throw new GsonSyntaxException(ex);
	 ...
	 * 使用者应该通过捕获GsonSyntaxException来处理可能的转换异常。
	 *
	...
	public JsonElement toJsonTree(Object obj, Type typeOfSrc) {
	    try {
	        // 尝试将对象转换为JsonElement，可能抛出GsonSyntaxException。
	        return toJsonTreeWithException(obj, typeOfSrc);
	    } catch (GsonSyntaxException ignored) {
	        // 捕获并忽略GsonSyntaxException，允许方法在异常情况下优雅地返回null。
	    }
	    return null;
	}
	 ...
	 * @throws GsonSyntaxException GsonSyntaxException
	 */
	public JsonElement toJsonTreeWithException(Object obj)  throws GsonSyntaxException {
		try {
	 ...
	 * 如果转换失败，将抛出GsonSyntaxException，以便调用者可以了解转换过程中发生的问题。
	 *
	 * @param obj 要转换为Json的Java对象。
	 * @param typeOfSrc Java对象的类型信息，用于更精确的转换。
	 * @return 转换成功的JsonElement。
	 * @throws GsonSyntaxException 如果转换过程中发生错误，则抛出此异常。
	 */
	public JsonElement toJsonTreeWithException(Object obj, Type typeOfSrc) throws GsonSyntaxException {
	    try {
	        // 尝试使用Gson将对象转换为JsonElement。
	        return gson().toJsonTree(obj, typeOfSrc);
	    } catch (Exception ex) {
	        // 捕获转换过程中可能出现的任何异常，并抛出GsonSyntaxException。
	        throw new GsonSyntaxException(ex);
	    }
	}
	...
	public <T> T fromJsonWithException(String json,Class<T> classOfT) throws GsonSyntaxException{
		try {
	...
	public <T> T fromJsonWithException(JsonElement json,Class<T> classOfT) throws GsonSyntaxException{
		try {
			return gson().fromJson(json, (Type) classOfT);
		} catch(Exception ex) {
			throw new GsonSyntaxException(ex);
	...
	public <T> T fromJsonWithException(String json, Type type) throws GsonSyntaxException{
		try {
	...
	public <T> T fromJsonWithException(JsonElement json, Type type) throws GsonSyntaxException{
		try {
			return gson().fromJson(json ,type);
		} catch(Exception ex) {
			throw new GsonSyntaxException(ex);
